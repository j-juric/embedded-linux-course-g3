...
static ssize_t encrypter_write(struct file *filp, const char *buf, size_t len, loff_t *f_pos)
{
    ...
    //printk(KERN_INFO "Envoked write function\n");
    /* Reset memory. */
    memset(encrypter_buffer, 0, BUF_LEN);
    ...
    else
    {
        if(mode[0] == 'd'){
                ...
                /*for(j=0;j<shift;j++){    	
                    pom = encrypter_buffer[0];

                    for(i=0; i < BUF_LEN-2; i++){
                        encrypter_buffer[i] = encrypter_buffer[i+1];
                    }
                    encrypter_buffer[BUF_LEN-1] = pom; */

                for(i=0; i < shift; i++){
                    char temp = encrypter_buffer[0];
                    unsigned char j;
                    for (j = 0; j < len - 2; j++)
                        encrypter_buffer[j] = encrypter_buffer[j + 1];
                    encrypter_buffer[len-2] = temp;
                    encrypter_buffer[len-1]='\0';
                }
                //printk(KERN_INFO "%s\n",encrypter_buffer);
		 }
        return len;
